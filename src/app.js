...
const package_json=(parameter)=>`
{
    ...
    ${
        ...
        "repository": {
            "type": "git",
            "url": "${"git_repository"in parameter?parameter.git_repository:"\t"}"
          },     
        `:""
    }
...
inquirir.prompt([
    {
        ...
        default:"Carpeta_"+new Date().toLocaleString().replace(/[\s:\/]/g,"-")
    ...
    */
    ...
    },
    {
        type:'list',
        name:'npm_install',
        message:'¿Deseas que se instale automaticamente las dependencias requeridas?',
        choices:[
            "Si",
            "No"
        ]
        ...
        type:'list',
        name:'open_proyect',
        message:'Cual es el comando para importar tu proyecto en tu editor de codigo : ?',
        choices:["code",'mate','subl','atom','Ninguna de las anteriores']
    }
]).then(async data=>{
    const  {package_name:proccess_package_name,author,license,git_repository,version,description} = data
    const package_name=proccess_package_name == "default(working_dir)"?data.working_dir:proccess_package_name
    const {stdout} = await exec(`mkdir ${data.working_dir}`)
    //data["npm_init"] == 'Automatico'?
    await exec(`git init`,{cwd:data.working_dir})
    await exec(`touch package.json && echo '${package_json({package_name,author,license,git_repository,author,version,description})}'>package.json`,{cwd:data.working_dir})
    ...
    for(let item  of data.archivos_primarios){
        if(item == '.babelrc')
        {
            await exec (`touch ${item} && echo '${data_babel_rc}'> ${item}`,{cwd:data.working_dir})
        }
        if(item == '.gitignore')
        {
            await exec (`touch ${item} && echo '${data_git_ignore}'> ${item}`,{cwd:data.working_dir})
        }
        if(item == '.env')
        {
            await exec (`touch ${item} && echo 'handler=express'>${item}`,{cwd:data.working_dir})
        }
        await exec (`touch ${item}`,{cwd:data.working_dir})
    }
    await exec('git clone https://github.com/WasauskyOK/src.git',{cwd:data.working_dir})
    if(data.npm_install == 'Si'){
        console.log('Instalando dependencias necesarias ...')
        await exec('npm install',{cwd:data.working_dir})
        console.log('Dependencias instaladas ... \n')
    }
    console.log("Ejecutando en desarrollo (npm run dev) en una nueva terminal :D")
    await exec(`gnome-terminal -- bash -c 'npm run dev; exec zsh || exec bash'`,{cwd:data.working_dir}) 
